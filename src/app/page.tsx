...
export default function Page() {
  ...
  const handleAddItem = (section: Section, draft: ItemDraft) => {
    const item = buildItem(crypto.randomUUID(), draft);

    return section === "income" ? addIncomeItem(item) : addExpenseItem(item);
  };

  const handleUpdateItem = (section: Section, id: string, draft: ItemDraft) => {
    const item = buildItem(id, draft);
  ...
  const handleAddCategory = (section: Section, draft: CategoryDraft) => {
    const category = buildCategory(crypto.randomUUID(), draft);
  ...
  const handleUpdateCategory = (
    ...
    const category = buildCategory(id, draft);
...
function buildItem(id: string, draft: ItemDraft) {
  const { name, amount, frequency, categoryId, notes } = draft;

  return createItem({ id, name, amount, frequency, categoryId, notes });
}

function buildCategory(id: string, draft: CategoryDraft) {
  const { name } = draft;
